...
def get_https_proxy():
    res = requests.get("http://10.1.18.35:8000/?types=0&count=1&protocol=1&country=国内")
    JS = json.loads(res.text)
    if JS:
        proxies = ['https://'+proxy[0]+':'+str(proxy[1]) for proxy in JS]
        if proxies:
            return proxies[0]
...
class ProxyMiddleware(object):
    ...
    def getproxy(self):
        ...
        while True:
            ...
            if self.proxys:
                break
        logger.debug("目前可用代理数为%s"%(len(self.proxys)))
        return self.proxys
    ...
    def process_request(self, request, spider):
        ...
        if 'https' in request.url:
            ...
            request.meta['proxy'] = https_proxy if https_proxy else None
            return
        try:
    ...
    def process_exception(self, request, exception, spider):
        ...
        """
        logger.warning("错误类型%s" % exception)
