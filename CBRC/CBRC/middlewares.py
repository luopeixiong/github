...
#            print("目前可用代理ip为%s"%(self.proxys))
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status != 200 \
            ...
            retry_times = new_request.meta['retry_times'] = new_request.meta.get('retry_times', 0)
            if retry_times < 3:
                return new_request
            else:
                msg = '%s is try more than maxtrytimes:3' % response.url
                scrapy.log.msg(msg, level=log.ERROR)
        else:
            return response
