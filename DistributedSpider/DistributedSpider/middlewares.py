...
def get_proxy():
    ...
    while True:
        try:
            res = requests.get("http://10.1.18.35:8000/?&count=50&country=%E5%9B%BD%E5%86%85")
            JS = execjs.eval(res.text)
            proxies = ['http://'+proxy[0]+':'+str(proxy[1]) for proxy in JS]
            if proxies:
                break
        # try:
        #     res = requests.get("http://piping.mogumiao.com/proxy/api/get_ip_bs?appKey=663f5c918b464aec8a5ee83b2f576dbe&count=20&expiryDate=0&format=1")
        #     JS = json.loads(res.text)['msg']
        #     proxies = ['http://'+proxy['ip']+':'+str(proxy['port']) for proxy in JS]
        #     if proxies:
        #         break
...
def delete_proxy(proxy):
    if (int(time.time())-Start) % 300 == 1:
        requests.get("http://10.1.18.35:8000/delete?country=%E5%9B%BD%E5%A4%96")
...
#            print("目前可用代理ip为%s"%(self.proxys))
    ...
    def process_request(self, request, spider):
        ...
        try:
            if self.proxys and last<300:
                if request.meta.get('proxys')==True:
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status != 200 \
            ...
            logger.info("response status not in spider.website_possible_httpstatus_list Status:%s" % response.status)
            try:
                proxy = random.choice(self.proxys)
            except IndexError:
                self.getproxy()
