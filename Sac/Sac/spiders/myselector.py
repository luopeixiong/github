...
class Re(object):
    ...
    def regex1(Regex,txt):
        result = re.compile(Regex).search(txt)
...
#                print(content)
    ...
    def datelist(start,end,formats="%Y%m%d"):
        ...
        date_l=[datetime.datetime.strftime(x,formats) for x in list(pd.date_range(start=start, end=end))]
        return date_l
